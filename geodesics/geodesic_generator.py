...
class GeodesicGenerator:
    def __init__(self, metric_space):
        ...
        # ^i_jk u^m u^n
        # ^i_k u^n
        Guu = sp.tensorcontraction(sp.tensorcontraction(sp.tensorproduct(metric_space.christ, uvec, uvec), (1, 3)),
                                   (1, 2))
    ...
    def calc_geodesic(self, tv0: TangentVector, t_span, n_pts):
        return Geodesic(solve_ivp(
            ...
            args=(self.Guu_np,) + tuple(self.metric_space.param_values[symbol] for symbol in self.metric_space.params),
            dense_output=True
        ))
